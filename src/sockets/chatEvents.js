...
const registerChatEvents = function(fastify, io, socket, onlineUsersMap){
    ...
    socket.on('createGroup', async(groupName)=>{
        try{
            ...
            const roomId = `room::${groupName}`;
            const message = 'group created';
            const aadMeta = {room:roomId, user:'__', timestamp};
            const encrypted = encryptMessage(roomId, message, aadMeta)
            const {roomDocPlain, roomDocEncrypted} = groupTemplate(groupName, timestamp, userId, message, encrypted);
            ...
            socket.emit('updateFeed', {id:roomId, ...roomDocPlain});
    ...
    socket.on('joinExistingGroup', async(groupId)=>{
        try{
            ...
            const aadMeta = {room:groupId, user, timestamp};
    ...
    socket.on('acceptRequest', async(friendId)=>{
        ...
        const user = socket.user.username;
        const timestamp = new Date().toISOString();
        const roomId = helperFunctions.makePrivateRoomId(userId, friendId);
        const message = `we are now friends!`;
        const aadMeta = {room:roomId, user:'__', timestamp};
        const encrypted = encryptMessage(roomId, message, aadMeta);
        const friendSocket = onlineUsersMap.get(friendId);
        const {roomDocPlain, roomDocEncrypted} = privateRoomTemplate(userId, friendId, roomId, timestamp, message, encrypted);
        try{
            const result = await fastify.rabbit.publishTaskRpc({
                type:'acceptRequest',
                payload:{
            ...
            const roomDoc = {id:roomId, ...roomDocPlain};
            helperFunctions.addFriendToCache(userId, friendId, cachedFriends);
